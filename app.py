...
def calculate_word_entropy(word, letter_frequencies):
    """Calculate the entropy of a word based on the frequency of its letters."""
    entropy = 0
    for letter in word:
...
def get_next_guess(guess_history, evaluation_history, possible_words):
    ...
    # Calculate the entropy of each possible word
    word_entropies = {word: calculate_word_entropy(word, letter_frequencies) for word in possible_words}

    # Sort possible words by their entropy
    sorted_words = sorted(possible_words, key=lambda word: word_entropies[word], reverse=True)
    ...
    return sorted_words[0]  # Return the best guess based on entropy
