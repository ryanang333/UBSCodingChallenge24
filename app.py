...
def get_next_guess(guess_history, evaluation_history, possible_words):
    ...
    # Use a more sophisticated method to determine the next guess
    # This method takes into account the frequency of each letter in the possible words
    # and the position of each letter in the word
    next_guess = max(sorted_words, key=lambda word: sum(letter_frequencies[letter] * (i + 1) for i, letter in enumerate(word)))
    return next_guess
