...
class RuleMatch(Rule):
    ...
    def apply(self, words, matched_counts):
        return [word for word in words if word[self.i] == self.letter]
...
class RuleContainsElsewhere(Rule):
    ...
    def apply(self, words, matched_counts):
        return [word for word in words if self.letter in word
...
class RuleExcludedLetter(Rule):
    ...
    def apply(self, words, matched_counts):
        filtered_words = []
        for word in words:
            if matched_counts[self.letter] == 0 and self.letter in word:
                continue
...
def wordle_game():
    ...
    print(len(guess_history), evaluation_history)
    wordle = Wordle()
   
    next_guess = FIRST_WORD

    for guess, evaluation in zip(guess_history, evaluation_history):
        rules, matched_counts = wordle.parse_rule_codes(evaluation, guess)
        wordle.apply_rules(rules, matched_counts)

    if len(guess_history) > 0:
        next_guess = wordle.get_test_word()
